...
public class FieldWorkerLoginTask extends AsyncTask<Boolean, Void, Result> {
	...
	private void invokeWebService() throws AuthenticationException, ClientProtocolException, IOException, XmlPullParserException {
		...
		String username = settings.getString(ServerPreferencesActivity.OPENHDS_KEY_USERNAME,
				((FieldWorkerLoginActivity) listener).getString(R.string.username));
		String password = settings.getString(ServerPreferencesActivity.OPENHDS_KEY_PASSWORD,
				((FieldWorkerLoginActivity) listener).getString(R.string.password));
		String url = settings.getString(ServerPreferencesActivity.OPENHDS_KEY_SERVER,
				((FieldWorkerLoginActivity) listener).getString(R.string.default_openhdsserver));
	...
	private boolean createFieldWorker(FieldWorker fieldWorker) {
		Cursor cusor = resolver.query(OpenHDS.FieldWorkers.CONTENT_ID_URI_BASE, new String[] { OpenHDS.FieldWorkers._ID },
				OpenHDS.FieldWorkers.COLUMN_FIELDWORKER_EXTID + " = ?", new String[] { fieldWorker.getExtId() }, null);
